...
export default async function RelatoriosPage({
  ...
  const [{ data: obra }, { data: todosOsDiarios }] = await Promise.all([
    supabase
      .from('obras')
      .select(
        'id, nome, endereco, responsavel_tecnico, art_rrt, numero_contrato, data_inicio, previsao_termino',
      )
      .eq('id', id)
      .single(),
    supabase
      ...
      .eq('obra_id', id)
      .order('data', { ascending: true }), // ASC para numeração sequencial (1 = mais antigo)
  ])

  if (!obra) notFound()

  // ─── Contagem de fotos por diário ───────────────────────────────────────────
  ...
  const fotosMap = (fotosData ?? []).reduce(
    (acc, f) => {
      acc[f.diario_id] = (acc[f.diario_id] ?? 0) + 1
      return acc
    },
    {} as Record<string, number>,
  )

  // ─── Monta lista com número sequencial e contagem de fotos ──────────────────
  ...
  // ─── Dados para o PDF consolidado (só quando de/ate presentes) ──────────────
      ...
      ids.length > 0
        ? await Promise.all([
            supabase
              ...
              .select('diario_id, funcao, quantidade, horas, subempreiteira_nome')
              .in('diario_id', ids),
            supabase
              .from('diario_equipamentos')
              .select('diario_id, nome, status, horas_uso')
              .in('diario_id', ids),
            supabase
              .from('diario_servicos')
              .select('diario_id, descricao, percentual_conclusao, localizacao')
              .in('diario_id', ids),
            supabase
              .from('diario_materiais')
              .select('diario_id, item, quantidade, unidade, fornecedor, nota_fiscal')
              .in('diario_id', ids),
            supabase
              .from('diario_ocorrencias')
              .select('diario_id, descricao, classe')
              .in('diario_id', ids),
          ])
      ...
      diarios: (diariosRange ?? []).map((d) => ({
        ...
        servicos: (serv ?? [])
          ...
          .map((r) => ({
            ...
            percentual_conclusao: r.percentual_conclusao,
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="mb-5">
        <div className="flex items-center gap-2 text-sm text-gray-400">
          <Link href="/obras" className="hover:text-gray-600">
            Obras
          </Link>
          <span>/</span>
          <Link href={`/obras/${id}`} className="hover:text-gray-600">
            {obra.nome}
          </Link>
          <span>/</span>
          <span className="text-gray-700 font-medium">Relatórios</span>
        </div>
      </div>
